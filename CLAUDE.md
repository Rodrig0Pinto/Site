...
- **Fundos fotográficos (v7):** `img/bg-hero.webp` (32 KB, biblioteca clássica) e `img/bg-citacao.webp` (23 KB, livros jurídicos) — fotos Unsplash (licença livre comercial) com **véu azul-marinho embutido** na imagem; aplicadas via CSS só em ≥769px (mobile mantém gradientes). Para trocar, reprocessar com o mesmo véu (blend navy ~75%).
...
- **Cache-busting obrigatório:** `/css/*` e `/js/*` têm cache de 7 dias e `/img/*` de 30 dias no navegador. **Sempre que alterar `style.css`, incrementar o `?v=N`** nas referências de todas as páginas (atual: `css/style.css?v=10`; logo com `?v=2`) — sem isso, visitantes recorrentes ficam até uma semana vendo o estilo antigo (foi o caso do título dourado em 18/07/2026).
