...
| Layout | Single-page: Hero, Números, **Boutique** (posicionamento: poucos casos/alta complexidade), Áreas, Sobre, Diferenciais, Citação, Notícias, Artigos, FAQ, Contato |
...
- **Cache-busting obrigatório:** `/css/*` e `/js/*` têm cache de 7 dias e `/img/*` de 30 dias no navegador. **Sempre que alterar `style.css`, incrementar o `?v=N`** nas referências de todas as páginas (atual: `css/style.css?v=12`; logo com `?v=2`) — sem isso, visitantes recorrentes ficam até uma semana vendo o estilo antigo (foi o caso do título dourado em 18/07/2026).
