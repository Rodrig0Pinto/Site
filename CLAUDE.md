...
- **Contador de visitas (D1)**: banco `rp-estatisticas` (binding `ESTATISTICAS`), provisionado pelo workflow `infra-d1.yml`; `functions/api/hit.js` grava **apenas contadores agregados** (dia, página, país, região, cidade) — sem IP, cookie ou identificador. Sinalizador em `js/main.js` e `js/blog.js`, 1 contagem por página por sessão; robôs descartados.
